...
function usp_phone_display() {
	...
	$phone_display = array(
		...
		'disable' => array(
			'value' => 'disable',
			'label' => esc_html__('Do not auto-display submitted phone', 'usp')
		),
	);
...
function usp_auto_display_options($item) {
	...
	$usp_phone_display = usp_phone_display();
	...
	if ($item === 'images') {
		...
		$array = $usp_phone_display;
		$key = 'auto_display_phone';
	...
	if (($tmp['default_options'] == '1') || (!is_array($tmp))) {
		...
		$arr = array(
			...
			'auto_email_markup'   => '<p><a href="mailto:%%email%%">'. esc_html__('Email', 'usp') .'</a></p>',
			 'auto_phone_markup'   => '<p>Phone : %%phone%%'. esc_html__('Phone', 'usp') .'</p>',
...
function usp_validate_options($input) {
	...
	$usp_phone_display = usp_phone_display();
	if (!isset($input['auto_display_phone'])) $input['auto_display_phone'] = null;
	if (!array_key_exists($input['auto_display_phone'], $usp_phone_display)) $input['auto_display_phone'] = null;
	...
	$input['usp_phone']           = wp_filter_nohtml_kses($input['usp_phone']);
	...
	$input['auto_phone_markup']   = wp_kses_post($input['auto_phone_markup'],   $allowedposttags);
	...
	if (!isset($input['usp_phone_html'])) $input['usp_phone_html'] = null;
	$input['usp_phone_html'] = ($input['usp_phone_html'] == 1 ? 1 : 0);
						...
						<div class="toggle<?php if (!isset($_GET['settings-updated'])) { echo ' default-hidden'; } ?>">
							...
							<div class="mm-table-wrap mm-table-less-padding">
								<table class="widefat mm-table">
									...
									<tr>
										<th scope="row"><label class="description" for="usp_options[phone]"><?php esc_html_e('User Phone', 'usp'); ?></label></th>
										<td>
											<select name="usp_options[usp_phone]" id="usp_options[usp_phone]">
												<option <?php if ($usp_options['usp_phone'] == 'show') echo 'selected="selected"'; ?> value="show"><?php esc_html_e('Display and require', 'usp'); ?></option>
												<option <?php if ($usp_options['usp_phone'] == 'optn') echo 'selected="selected"'; ?> value="optn"><?php esc_html_e('Display but do not require', 'usp'); ?></option>
												<option <?php if ($usp_options['usp_phone'] == 'hide') echo 'selected="selected"'; ?> value="hide"><?php esc_html_e('Disable this field', 'usp'); ?></option>
											</select>
										</td>
									</tr>
									<tr>
							...
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									...
									<tr>
										...
										<th scope="row"><label class="description" for="usp_options[auto_display_phone]"><?php esc_html_e('Phone Auto-Display', 'usp'); ?></label></th>
										<td>
											<span class="mm-item-desc"><?php esc_html_e('Auto-display user-submitted phone:', 'usp'); ?></span>
											<?php usp_auto_display_options('phone') ; ?>
										</td>
									
									<tr>
										<th scope="row"><label class="description" for="usp_options[auto_phone_markup]"><?php esc_html_e('Phone Markup', 'usp'); ?></label></th>
										<td><textarea class="textarea" rows="3" cols="50" name="usp_options[auto_phone_markup]"><?php echo esc_attr($usp_options['auto_phone_markup']); ?></textarea> 
										<div class="mm-item-caption"><?php esc_html_e('Markup to use for the submitted phone number (when auto-display is enabled). Can use', 'usp'); ?> 
										<code>%%phone%%</code> <?php esc_html_e('and', 'usp'); ?> <code>%%title%%</code>.</div></td>
									</tr>
									<tr>
