...
function usp_checkForPublicSubmission() {
	...
	if (isset($_POST['user-submitted-post'], $_POST['usp-nonce']) && !empty($_POST['user-submitted-post']) && wp_verify_nonce($_POST['usp-nonce'], 'usp-nonce')) {
		...
		$phone    = isset($_POST['user-submitted-phone'])    ? sanitize_text_field($_POST['user-submitted-phone'])         : '';
		...
		$result = usp_createPublicSubmission($title, $files, $ip, $author, $url, $email, $phone,$tags, $captcha, $verify, $content, $category);
...
function usp_createPublicSubmission($title, $files, $ip, $author, $url, $email, $phone, $tags, $captcha, $verify, $content, $category) {
	...
	if (isset($usp_options['usp_phone'])     && ($usp_options['usp_phone']     != 'hide') && !usp_validatePhone($phone))  $newPost['error'][] = 'required-phone';
	...
	if ($newPost['id']) {
		...
		if (!empty($phone))    update_post_meta($post_id, 'user_submit_phone', $phone);
...
function usp_validatePhone($phone) {
    
	$phone= str_replace(" ", "", $phone); 
	if (preg_match("/^[0-9]{9}$/",$phone) || preg_match("/^\+[0-9]{11}$/",$phone) || preg_match("/^[0][0-9]{9}$/",$phone) || preg_match("/^[0]{2}[0-9]{11}$/",$phone)) 
		{ return true; }
	else 
		{return false;}
/*
	^[0-9]{9}$ //691541062, 523857754
	^\+[0-9]{11}$ //+48691541062, +48523857754
	^[0][0-9]{9}$ //0691541062, 0523857754
	^[0]{2}[0-9]{11}$ // 0048691541062, 00523857754
*/
	
	$bad_stuff = array("\r", "\n", "mime-version", "content-type", "cc:", "to:");
	foreach ($bad_stuff as $bad) {
		if (strpos(strtolower($phone), strtolower($bad)) !== false) {
		return false;	
		}
	}
	return true;
...
function usp_error_message() {
	...
	if (isset($_GET['usp-error']) && !empty($_GET['usp-error'])) {
		...
		foreach ($error_array as $e) {
			...
			elseif ($e == 'required-phone')      $error[] = esc_html__('User phone required', 'usp');
